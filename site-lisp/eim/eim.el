...
;; Version: $Id: eim.el,v 1.2 2006/07/30 15:32:25 ywb Exp ywb $
;; Keywords:
...
(defun eim-line-content (&optional seperaters omit-nulls)
  ...
  (let ((items   (split-string
    ...
    (if omit-nulls
        (remove-if 'eim-string-emptyp items)
      items)))
...
(defface eim-tooltip-face '((((class color))
                             :inherit tooltip))
...
(defun eim-show-tooltip (text)
  (let ((pos (eim-frame-posn-at-point))
        (fg (face-attribute 'eim-tooltip-face :foreground))
        (bg (face-attribute 'eim-tooltip-face :background))
        (params tooltip-frame-parameters)
        left top)
    (setq left (+ (car pos) (frame-parameter (selected-frame) 'left))
          top (+ (cdr pos) (frame-parameter (selected-frame) 'top)))
    (when (stringp fg)
      (setq params
            (append params `((foreground-color . ,fg)
                             (border-color . ,fg)))))
    (when (stringp bg)
      (setq params
            (append params `((background-color . ,bg)))))
    (setq params
          (append params `((left . ,left)
                           (top . ,top))))
    (x-show-tip text nil params eim-tooltip-timeout
                0 (frame-char-height))))
