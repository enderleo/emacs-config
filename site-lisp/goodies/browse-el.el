...
(defun wcy-browse-el-push-point-mark()
  (if (eq (length wcy-browse-el-point-ring) wcy-browse-el-point-ring-max-number)
      (setq wcy-browse-el-point-ring (butlast wcy-browse-el-point-ring)))
...
(defun wcy-browse-el-find-funtion ()
  (interactive)
  (let ((fun (find-function-at-point)))
    (if fun
        (progn
...
(defun wcy-browse-el-key-binding ()
  (interactive)
  (local-set-key (kbd "<f6>") 'wcy-browse-el-find-funtion)
  (local-set-key (kbd "<f5>") 'wcy-browse-el-go-back))


(add-hook 'lisp-mode-hook 'wcy-browse-el-key-binding)
(provide 'browse-el)
