...
(defvar dna-nt-sequence-start-regexp
  "^\\(>\\|ID\\|ORIGIN\\|DNA\\)"
  "A regexp which matches the start of a sequence.")
...
(defvar dna-mode-map
  (let ((map (make-sparse-keymap)))
    ...
    (define-key map "\C-cn"	'dna-goto-base)
...
(defun dna-goto-base (pos)
  (interactive "nGo to base: ")
  (dna-beginning-of-nt-sequence)
  (next-line)
  (dna-forward-base pos))

(defun dna-beginning-of-sequence-p ()
  "At the beginning of a sequence?"
  (looking-at dna-sequence-start-regexp))

(defun dna-beginning-of-nt-sequence ()
  "Move to the start of the sequence or the buffer."
  (interactive)
  (goto-char
   (or
    (search-backward-regexp dna-nt-sequence-start-regexp (point-min) t)
    (point-min))))
